...
export function parseRows(allRows, opts = {}) {
  const OPT = { fb: true, fixDeger: true, prec: true, ...opts };
  ...
  for (let ri = 0; ri < rows.length; ri++) {
    ...
    // "%4,34" / "-%2,90" biçimindeki yüzdeleri "4,34%" biçimine çevir
    const toks = r.tokens.map(t => /^[-(]?%\d[\d.,]*\)?$/.test(t.s) ? { ...t, s: t.s.replace(/^([-(]?)%/, "$1") + "%" } : t);
    const nums = [];
    for (const t of toks) {
    ...
    // "Finansman Bonosu Toplam 61.000.000 ..." gibi: sayılardan önceki metin Toplam ile bitiyorsa ara toplam satırı
    { const lead = []; for (const t of toks) { if (NUM_TOK.test(t.s) || DATE_TOK.test(t.s)) break; lead.push(t.s); }
      if (lead.length && /\bTOPLAMI?\s*:?$/.test(fold(lead.join(" ")))) { cur = null; continue; } }
    ...
    if (nums.length >= 2) {
      ...
      // yedek ağırlık: satır sonundaki yüzde değeri (tutar yanlış okunursa kullanılır)
      let fb = null; { let k2 = nums.length; while (k2 > 0 && Math.abs(nums[k2 - 1].v) <= 100.0001 && nums[k2 - 1].dec) { fb = nums[k2 - 1].v; break; } }
      cur = { isin: isins[0] || "", code, words, section, oran, deger, fb, sig: `${r.page || 0}|${toks.map(t => t.s).join(" ")}` };
  ...
  for (const r of recs) if (Number.isNaN(r.oran)) {
    ...
    const calc = ident && FTD && r.deger ? Math.round(r.deger / FTD * 1e8) / 1e6 : null;
    r.oran = calc === null ? null : Math.abs(calc) <= 100 ? calc : (OPT.fb ? (r.fb ?? null) : null);
  }
  if (FTD && OPT.fixDeger) for (const r of recs) if (r.deger && Math.abs(r.deger) > FTD * 1.02 && r.oran !== null && !Number.isNaN(r.oran)) r.deger = Math.round(r.oran / 100 * FTD * 100) / 100;
  ...
  if (FTD && OPT.prec) {
    ...
    if (cmp >= 2 && agree / cmp >= 0.8) {
      // yüzde yerine oran (0,0151 = %1,51) yazılmış satırlar
      for (const h of out) if (h.deger && h.oran) { const calc = h.deger / FTD * 100, r = calc / h.oran; if (r > 97 && r < 103) h.oran = Math.round(calc * 1e6) / 1e6; }
...
function clean(recs) {
  ...
  for (const r of recs) {
    ...
    if (kod === r.isin) kod = "";
    const curCode = /^(TL|TRY|USD|EUR|GBP|CHF|JPY|XAU|ALTIN)$/.test(kod) ? kod : "";
    if (curCode) kod = "";
    ...
    // sadece para birimi yazan satırlar (ör. VİOP nakit teminatı "TRY"): bölüm adıyla tut
    if (!r.isin && !kod && !ad && curCode && r.deger) ad = `${normText(r.section) || "Nakit"} (${curCode})`;
...
// OCR raporları: birkaç okuma ayarını dene, toplamı %100'e en yakın olanı seç (sadece %90–110 kabul)
export function parseOcrRows(rows) {
  // OCR kırıntılarını at: tek başına "o", "Oo", "(o", "|", "—", "©" gibi tokenlar
  rows = rows.map(r => ({ ...r, tokens: (r.tokens || []).filter(t => !/^[\(\)\[\]|—–_©®‘’'"“”=;:.,~*•°oO0]{1,3}$/.test(t.s) || /^\d$/.test(t.s)) }));
  const variants = [{}, { fb: false }, { fixDeger: false }, { fb: false, fixDeger: false }, { fb: false, fixDeger: false, prec: false }];
  let best = null;
  for (const v of variants) {
    const h = parseRows(rows, v), q = quality(h), dist = Math.abs(100 - q.toplamOran);
    if (!best || dist < best.dist) best = { h, q, dist, v };
  }
  // %110 üstü: yanlış satır okunmuş → reddet. %75–110: kabul (85 altı "kısmi" olarak işaretlenir)
  const ok = best && best.q.toplamOran >= 75 && best.q.toplamOran <= 110;
  return ok ? { holdings: best.h, ...best.q, ok: true } : { holdings: [], ...(best ? best.q : {}), ok: false };
}
// Türev pozisyonları (vadeli, opsiyon, forward, swap) varlık değil, nominal pozisyondur: toplam kontrolüne girmez
export const isDerivative = h => /^[FO]_/.test(h.kod || "") || (/(VIOP ISLEM|VOB SOZLESME|VIOP SOZLESME|OPSIYON|FORWARD|SWAP|VADELI ISLEM SOZ)/.test(fold(h.tur || "")) && !/TEMINAT/.test(fold(h.tur || "")));
export function quality(holdings) {
  const s = holdings.filter(h => !isDerivative(h)).reduce((a, h) => a + (h.oran || 0), 0);
